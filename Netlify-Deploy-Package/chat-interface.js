...
- 请严格遵守，始终以${langInfo.name}思考和回复`;
            ...
            console.log('📋 最终 System Prompt 检查:', {
                ...
                systemPromptEnds: systemPrompt.slice(-200) // 打印最后200个字符
            });
                                ...
                                try {
                                    ...
                                    if (chatConfig.showTimestamp && msg.time) {
                                        ...
                                        if (timestamp) {
                                            ...
                                            return `<div style="position: absolute; bottom: -18px; right: 4px; font-size: 11px; color: #999; white-space: nowrap;">${timeStr}</div>`;
                                        }
                                    }
                                } catch (e) {}
                                return '';
...
async function executeReplyTask(task) {
    ...
    const conversationHistory = recentMessages.map(msg => {
        ...
        if (msgType === 'pay-request' || msgType === 'pay-response' || msgType === 'pay-done') {
            // 解析并格式化JSON，让AI能识别
    ...
    if (lastUserMessage) {
        ...
        if (lastContent.includes('代付') || lastContent.includes('帮我付') || lastContent.includes('帮我买') || lastContent.includes('给我买')) {
            console.log('🔍 检测到用户请求代付，添加上下文提示');
            // 在对话历史末尾添加系统提示
            conversationHistory.push({
                role: 'system',
                content: '【系统提示】用户刚才请求你代付商品。请注意：如果对方发送过{"type":"pay-request"}的JSON，那是代付请求，不是转账请求！代付请返回{"type":"pay-done",...}，绝对不能用transfer！'
            });
        }
    }
...
`;
    ...
    try {
        ...
        try {
            ...
            const jsonMatch = aiReply.match(/\{[\s\S]*"type"\s*:\s*"(family-card|transfer|receive-family-card|pay-done)"[\s\S]*\}/);
            
            if (jsonMatch) {
                const jsonStr = jsonMatch[0];
                const jsonData = JSON.parse(jsonStr);
                        
                if (jsonData.type === 'family-card' || jsonData.type === 'transfer' || jsonData.type === 'receive-family-card' || jsonData.type === 'pay-done') {
